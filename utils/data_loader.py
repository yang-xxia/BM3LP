...
class ConvKBCorpus(Corpus):
    ...
    def get_batch(self, batch_num):
        ...
        pos_triples = self.train_indices[list(indices)]
        last_index = len(pos_triples)

        # 正样本
        self.batch_indices = np.zeros((last_index * (self.neg_num + 1), 3), dtype=np.int32)
        self.batch_values = np.zeros((last_index * (self.neg_num + 1), 1), dtype=np.float32)
            ...
            for n in range(self.neg_num):
                if n % 2 == 0:  #
                    ...
                    while (h_neg, r, t) in self.all_triples:
                        h_neg = np.random.randint(0, len(self.entity2id))
                    self.batch_indices[idx] = [h_neg, r, t]
                else:  # 换 tail
